...
SELECT department_name, count(*) as employees_no FROM employees 
INNER JOIN departments USING(department_id) GROUP BY(department_name);
-- 4. **Find the Department name with the Highest Average Salary**
SELECT department_name, round(avg(salary)) as highest_avg_salary  FROM employees 
INNER JOIN departments USING(department_id) GROUP BY(department_name)
ORDER BY(highest_avg_salary) DESC
LIMIT 1;
-- 5. **Count Employees Hired Each Year**
SELECT extract(YEAR FROM hire_date) as hired_year, count(*) as hired_employee FROM employees
GROUP BY(hired_year);
